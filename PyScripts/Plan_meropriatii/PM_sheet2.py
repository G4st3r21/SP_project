    ...
    for row in rows:
        if str(row[0].value)[:2] == 'СЦ':
            ...
            for obj in response_obj:
                id_resp_obj = cur.execute(f"SELECT FROM RESPONSE_OBJ * WHERE resp_obj like '{obj}'").fetchall()
                cur.execute(f"INSERT INTO INDICATORS_AND_RESPONSE_OBJ VALUES ({indicator_id}, {id_resp_obj[0]})")
