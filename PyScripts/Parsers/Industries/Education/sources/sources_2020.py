...
table_name = "12. Все источники.xlsx"
sheet_number = read_sheet_number()  # 5
start_row = read_first_str_number()  # 8

# cur, conn = db_conn()
sheet = load_workbook(table_name).worksheets[sheet_number]
suitable_rows = list(sheet.rows)[start_row:len(list(sheet.rows))-1]
...
for row in suitable_rows:
    ...
    # source_name prepare
    source_name = row[2].value
    while source_name[0] == " " or source_name[0] == "-":
        source_name = source_name[1:]

    # find suitable row
    if source_name in sources and row[3].value != 0:
        print(f"{id_} {code_events} {sources.index(source_name)} {row[3].value} {row[4].value}")
        id_ = id_ + 1
    # cur.execute()
# conn.commit()
