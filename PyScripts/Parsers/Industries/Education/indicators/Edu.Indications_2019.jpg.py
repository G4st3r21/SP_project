...
table_name = "Отчет_за_2019_г..xlsx"
...
for row in suitable_rows:
    ...
    if title_indicator is not None:
        ...
        # cur.execute(f"INSERT INTO INDICATORS2019 VALUES ({id_}, '{code_events}', {id_}, '{row[6].value}', {row[7].value}, {row[8].value})")
        print(f"  {id_} {title_indicator} {row[5].value}")
        # cur.execute(f"INSERT INTO NAMES_INDICATORS2019 VALUES ({id_}, '{title_indicator}', '{row[5].value}')")
