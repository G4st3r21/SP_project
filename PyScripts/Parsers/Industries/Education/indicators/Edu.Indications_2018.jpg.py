...
table_name = "Отчет за 2018_показатели_отв.xlsx"
...
for row in suitable_rows:
    ...
    if title_indicator is not None:
        ...
        cur.execute(f"INSERT INTO INDICATORS2018 VALUES ({id_}, '{code_events}', {id_}, '{row[6].value}', {row[7].value}, {row[8].value})")
        print(f"  {id_} {title_indicator} {row[5].value}")
        cur.execute(f"INSERT INTO NAMES_INDICATORS2018 VALUES ({id_}, '{title_indicator}', '{row[5].value}')")
        if (row[9].value != "") and (row[9].value is not None):
            print(f"     {id_} {row[9].value}")
            cur.execute(f"INSERT INTO REJECTION2018 VALUES({id_},'{row[9].value}'")
