...
def part(string):
    ...
    #         string = string[:string.find('-')] + ' ' + string[string.find('-'):]
    ...
    if len(parts) > 1:
        for i in range(len(parts) - 1):
            ...
            if parts[i + 1].endswith(',') or parts[i + 1].endswith(';') and parts[i + 1][0] in 'АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЭЮЯ':
...
Subprogram = SPTableArbitrary('subprogram' + year, cur, conn, schema='Municipalities')
MainEvent = SPTableArbitrary('main_event' + year, cur, conn, schema='Municipalities')
Event = SPTableArbitrary('event' + year, cur, conn, schema='Municipalities')
AllEvents = SPTableManyToMany('all_events' + year, cur, conn, schema='Municipalities')
ResponseObj = SPTable('response_obj', cur, conn)
ResponseFio = SPTableArbitrary('response_fio' + year, cur, conn, schema='Municipalities')
EventsResponseObj = SPTableManyToMany('events_response_obj' + year, cur, conn, schema='Municipalities')
EventsResponseFio = SPTableManyToMany('events_response_fio' + year, cur, conn, schema='Municipalities')
# ResponseMain = SPTableArbitrary('response_main' + year, cur, conn, schema='Municipalities')
