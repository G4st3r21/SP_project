...
def part(string):
    ...
    for j in range(len(str_list)):
        ...
        if str_list[j].endswith('\n') or str_list[j].endswith(';') or str_list[j].endswith('.'):
...
cols, rows, cur, conn = parser_init("2020.xlsx",
                                    sheet_number=2, first_str_number=8)
first_column = 1
year = str(2020)

Gosprogram = Gosprogram(cur, conn)
Subprogram = SPTableArbitrary('subprogram' + year, cur, conn, schema='Culture')
MainEvent = SPTableArbitrary('main_event' + year, cur, conn, schema='Culture')
Event = SPTableArbitrary('event' + year, cur, conn, schema='Culture')
AllEvents = SPTableManyToMany('all_events' + year, cur, conn, schema='Culture')
ResponseObj = SPTable('response_obj', cur, conn)
ResponseFio = SPTableArbitrary('response_fio' + year, cur, conn, schema='Culture')
EventsResponseObj = SPTableManyToMany('events_response_obj' + year, cur, conn, schema='Culture')
EventsResponseFio = SPTableManyToMany('events_response_fio' + year, cur, conn, schema='Culture')
# ResponseMain = SPTableArbitrary('response_main2016', cur, conn, schema='Culture')
