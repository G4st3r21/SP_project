...
cols, rows, cur, conn = parser_init("Отчет за 2016 год.xlsx", sheet_number=1, first_str_number=8)
first_column = 1
year = str(2016)

Gosprogram = Gosprogram(cur, conn)
Subprogram = SPTableArbitrary('subprogram' + year, 'title_subprog', cur, conn, schema='Information_Society')
MainEvent = SPTableArbitrary('main_event' + year, 'main_event', cur, conn, schema='Information_Society')
AllEvents = SPTableManyToMany('all_events' + year, cur, conn, schema='Information_Society')
ResponseObj = SPTable('response_obj', 'response_obj', cur, conn)
ResponseFio = SPTableArbitrary('response_fio' + year, 'fio', cur, conn, schema='Information_Society')
EventsResponseObj = SPTableManyToMany('events_response_obj' + year, cur, conn, schema='Information_Society')
EventsResponseFio = SPTableManyToMany('events_response_fio' + year, cur, conn, schema='Information_Society')
